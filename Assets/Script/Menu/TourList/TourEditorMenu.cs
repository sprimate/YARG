...
namespace YARG
{
    // (sprimate) This is a really hacky way to design a menu without having to actually design a menu and hook things up. 
    // This should be done in the canvas like all other menus, but for prototyping, I just didn't feel like setting that all up.
    public static class TourEditorMenu
    {
        // =====================================================================
        ...
        private static TourData _tourToEdit;
        ...
        private static int _pickerShowIndex = -1;
        ...
        private static Texture2D _rowOddTexture;
        ...
        private static PreviewContext _previewContext;
        private static CancellationTokenSource _previewCanceller;
        private static SongEntry _previewingSong;

        // Buffers so int fields can be typed into freely (e.g. cleared) without snapping.
        private static readonly Dictionary<TourUnlockCondition, string> _amountBuffers = new();
        ...
        public static async UniTask<bool> DeleteTour(TourData tour)
        {
            if (tour == null)
            {
                return false;
        ...
        private static async UniTask DeleteTourFromEditor()
        {
            if (_tourToEdit == null)
            {
                return;
            ...
            // A brand-new tour that was never saved has nothing on disk —
            // deleting it just means discarding the editor.
            if (FindExistingTourPath(tour.TourId) == null)
            {
                CloseEditor();
                return;
            }

            // Close the editor first — its OnGUI renders full-screen on top
            // of everything, including the confirmation dialog, so it has to
            // be hidden before the dialog can be seen and interacted with.
            CloseEditor();

            if (await DeleteTour(tour))
            {
                return;
            }

            // Deletion was cancelled (or failed) — reopen the editor so the
            // user doesn't lose their in-progress edits.
            EditTour(tour);
        }

        public static void CreateAndEditTour()
        {
        ...
        public static void EditTour(TourData tourData)
        {
            if (tourData == null)
            {
            ...
            // Editor navigation scheme: Green = save, Red = back, Yellow = delete.
            Navigator.Instance.PushScheme(new NavigationScheme(new()
            {
                ...
                new NavigationScheme.Entry(MenuAction.Yellow, "Menu.Common.Delete",
                    () => DeleteTourFromEditor().Forget()),
            }, false));
        }
        ...
        private static void SaveEditor()
        {
            ...
            try
            {
                path = SaveTourToFile(_tourToEdit);
            }
            catch (Exception ex)
            {
                Debug.LogError($"Cannot save tour: {ex.Message}");
                return;
            }

            Debug.Log($"Saved tour '{_tourToEdit.TourName}' to {path}");
        ...
        /// <summary>
        /// Writes a tour to disk. Overwrites the tour's existing file if one
        /// exists, otherwise creates a new file in the top song folder.
        /// Also used by <see cref="TourCloudMenu"/> when saving downloaded tours.
        /// </summary>
        internal static string SaveTourToFile(TourData tour)
        {
            string path = FindExistingTourPath(tour.TourId);
            if (path == null)
            {
                ...
                if (folders == null || folders.Count == 0)
                {
                    throw new InvalidOperationException(
                        "No song folders are configured. Add a song folder in the settings first.");
                }
        ...
        private static readonly Dictionary<(string, string), bool> _libraryLookupCache = new();

        private static bool IsSongInLibrary(string artist, string songName)
        {
            artist ??= "";
            songName ??= "";
        ...
        private static async void PlayPreview(SongEntry entry)
        {
        ...
        private static void DrawTourEditor(Rect area)
        {
            var tour = _tourToEdit;
        ...
        private static bool DrawShow(TourData tour, int showIndex)
        {
        ...
        private static void DrawSongPicker(Rect area)
        {
        ...
        private static void DrawBottomBar(Rect area)
        {
            ...
            GUI.backgroundColor = new Color(0.95f, 0.8f, 0.2f);
            if (GUILayout.Button("● Delete Tour", ButtonStyle(), GUILayout.Height(40),
                GUILayout.Width(300)))
            {
                DeleteTourFromEditor().Forget();
            }
        ...
        private static GUIStyle MissingLabelStyle()
        {
        ...
        private static GUIStyle RowStyle(int index)
        {
